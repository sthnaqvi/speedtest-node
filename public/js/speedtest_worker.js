...
var clientIp = ""; // client's IP address as
...
var settings = {
	...
	url_dl: "/v1/speedtest/garbage",
	url_ul: "/v1/speedtest",
	url_ping: "/v1/speedtest",
	url_getIp: "/v1/ip",
	...
	garbage_chunkSize: 100, // size of chunks (can be different if enable_quirks is active)
	...
	var testStream = function (i, delay) {
		setTimeout(
			function () {
				...
				xhr[i].open("GET", settings.url_dl + url_sep(settings.url_dl) + (settings.mpot ? "cors=true&" : "") + "r=" + Math.random() + "&ckSize=" + settings.garbage_chunkSize, true); // random string to prevent caching
				xhr[i].send();
			}.bind(this),
			1 + delay
		);
