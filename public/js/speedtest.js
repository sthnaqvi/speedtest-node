...
Speedtest.prototype = {
   ...
   *       dlURL:"garbage"   path to garbage or its replacement on the server
   *       ulURL:"ping"   path to ping or its replacement on the server
   *       pingURL:"ping"   path to ping or its replacement on the server. This is used to ping the server by this selector
   *       getIpURL:"ip"   path to ip or its replacement on the server
   *   }
   */
  ...
  onLoad: function () {
    this.worker = new Worker("js/ip_worker.js?r=" + Math.random());
    this.worker.onmessage = function (e) {
      if (e.data === this._prevData) return;
      else this._prevData = e.data;
      var data = JSON.parse(e.data);
